...
app.get("/api/menu/:role", (req, res) => {
  ...
  function adjustFilename(filename) {
    // فقط للملفات التي تحتاج g للقسم female
    const addG = ["cycle2.pdf", "cycle3.pdf", "teachers.pdf", "duties.pdf", "timings.pdf"];
    if (gender === "female" && addG.includes(filename)) {
  ...
  let menu = [];
  if (role === "student") {
    menu = baseStudentMenu.map(item => ({ ...item, filename: item.filename ? adjustFilename(item.filename) : undefined }));
  } else if (role === "staff") {
    menu = baseStaffMenu.map(item => ({ ...item, filename: item.filename ? adjustFilename(item.filename) : undefined }));
  } else {
...
function loadStudentsFromExcel() {
  ...
  dataRows.forEach((row) => {
    ...
    studentId = String(studentId).replace(/\s/g, "").trim();
    if (!studentId) return;
